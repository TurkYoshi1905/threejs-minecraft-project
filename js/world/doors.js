...
export class DoorManager {
  ...
  get(x, y, z) {
    ...
    if (!this.map.has(k)) this.map.set(k, { face: 0, open: false });
  ...
  serialize() {
    const out = {};
    for (const [k, v] of this.map) out[k] = { face: (v.face | 0) & 3, open: !!v.open };
    return out;
  }
  load(obj) {
    this.map.clear();
    ...
    for (const [k, v] of Object.entries(obj)) {
      if (v && typeof v === 'object') this.map.set(k, { face: (v.face | 0) & 3, open: !!v.open });
    }
  }
}
